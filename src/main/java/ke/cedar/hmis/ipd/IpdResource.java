...
public class IpdResource {
    ...
    @RolesAllowed({"RECEPTIONIST", "NURSE", "DOCTOR",
                   "ADMIN", "CEO", "CLINICAL_OFFICER"})
    public List<Bed> getBedsByWard(@PathParam("wardId") Long wardId) {
    ...
    public List<Bed> getAvailableBeds(@PathParam("wardId") Long wardId) {
    ...
    public Bed updateBedStatus(
        ...
        return ipdService.updateBedStatus(bedId, body.get("status"));
    }

    @POST
    @Path("/admit")
    @Transactional
    @RolesAllowed({"NURSE", "DOCTOR", "CLINICAL_OFFICER", "ADMIN"})
    public Response admitPatient(Map<String, String> body) {
        Admission admission = ipdService.admitPatient(
            ...
            body.get("doctor"),
            body.get("diagnosis"));
        return Response.status(201).entity(admission).build();
    }
    ...
    @RolesAllowed({"NURSE", "DOCTOR", "CLINICAL_OFFICER", "ADMIN"})
    public List<Admission> getByWard(@PathParam("wardId") Long wardId) {
